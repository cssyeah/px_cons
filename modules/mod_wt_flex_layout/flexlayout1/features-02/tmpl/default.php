...
$featureBgColor       = $styleconfig->get('feature_bg_color');
$featureBorderWidth   = $styleconfig->get('feature_border_width');
$featureBorderColor   = $styleconfig->get('feature_border_color');
$featureTextColor     = $styleconfig->get('feature_text_color');
$featurePadding       = $styleconfig->get('feature_padding');
$featureBorderRadius  = $styleconfig->get('feature_border_radius');
$featureMediaMaxHeigh = $styleconfig->get('feature_media_max_height');
$featureBtnType       = $styleconfig->get('feature_btn_type');

$featureStyle = '';
$featureStyle .= $featureBgColor?'background-color: '.$featureBgColor.';':'';
$featureStyle .= ($featureBorderWidth > 0)?'border: '.$featureBorderWidth.'px solid '. $featureBorderColor .';':'';
$featureStyle .= $featureBorderRadius?'border-radius: '.$featureBorderRadius.';':'';
$featureStyle .= $featureTextColor?'color: '.$featureTextColor.';':'';
$featureStyle .= $featureTextAlignment?'text-align: '.$featureTextAlignment.';':'';
$featureStyle .= $featurePadding?'padding: '.$featurePadding.';':'';
...
<style>
  .features-<?php echo $module->id; ?>  .feature-item-inner a {    
    <?php echo $modLinkColor?'color:'.$modLinkColor.';':''; ?>
  }

  .features-<?php echo $module->id; ?> .feature-item-inner a:hover,
  .features-<?php echo $module->id; ?> .feature-item-inner a:focus,
  .features-<?php echo $module->id; ?> .feature-item-inner a:active {
    <?php echo $modLinkHoverColor?'color:'.$modLinkHoverColor.';':''; ?>
  }
</style>

<div class="wt-flex-layout features features-<?php echo $module->id; ?> layout-02 <?php echo $modSpacingTop . ' ' . $modSpacingBottom; ?>" style="<?php echo $modStyle; ?>">
      ...
      <div class="feature-item-inner<?php echo $featureTextAlignment?' text-'.$featureTextAlignment.'':''; ?>" style="<?php echo $featureStyle; ?>">
        <?php if($item->feature_image) : ?>
          <span class="feature-media">
            <img src="<?php echo $item->feature_image; ?>" alt="" style="<?php echo $featureMediaMaxHeigh?'max-height: '.$featureMediaMaxHeigh:'max-height: auto' ?>;" />
          </span>
        <?php endif; ?>
