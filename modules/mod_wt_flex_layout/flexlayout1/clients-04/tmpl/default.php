...
<div class="wt-flex-layout clients clients-<?php echo $module->id; ?> layout-04 <?php echo $modSpacingTop . ' ' . $modSpacingBottom; ?>" style="<?php echo $modStyle; ?>">
  ...
  <div class="clients-list">
    <ul style="grid-template-columns: repeat(<?php echo $clientsPerRow; ?>, 1fr); background-color: <?php echo $clientBgColor; ?>; border: <?php echo $clientBorderWidth; ?>px solid <?php echo $clientBorderColor; ?>;">
    <?php foreach ($items as $item) { ?>
      <?php if($item->client_image) : ?>
      <li class="<?php echo ($itemCount % $clientsPerRow) === 0?'last-child':'';  ?>" style="background-color: <?php echo $clientBgColor; ?>; border: <?php echo $clientBorderWidth; ?>px solid <?php echo $clientBorderColor; ?>; padding: <?php echo $clientPadding; ?>px;">

        <?php if($item->client_image) : ?>
        <img src="<?php echo $item->client_image; ?>" alt="<?php echo $item->client_name; ?>" />
        <?php endif ?>

        <?php if($item->client_link_url) : ?>
          <a href="<?php echo $item->client_link_url; ?>" title="<?php echo $item->client_name; ?>" class="link-mask"></a>
        <?php endif ?>
    ...
    <?php $itemCount++; } ?>
      ...
      <li class="line-bottom" style="border-bottom: <?php echo $clientBorderWidth; ?>px solid <?php echo $clientBorderColor; ?>;">&nbsp;</li>
    </ul>
  </div>
